...
LEFT_SLOT_ANGLES = tuple((angle - 180) % 360 for angle in RIGHT_SLOT_ANGLES)
...
RP6_COMPATIBLE = "retroidpocket,rp6"
...
def device_compatible():
    try:
        return Path("/sys/firmware/devicetree/base/compatible").read_bytes().split(b"\0")[0].decode()
...
def data_record(command, words):
    payload = b"".join(int(word).to_bytes(2, "big") for word in words)
    return COMMAND.pack(ABI_VERSION, command, len(payload), payload.ljust(58, b"\0"))
...
class RangeTracker:
    ...
    def __init__(self, stick, center):
        ...
        self.legacy_targets = LEFT_SLOT_ANGLES if stick == "left" else RIGHT_SLOT_ANGLES
    ...
    def _legacy_candidate(self):
        triples = [None] * 8
        selected = [None] * 8
        for record in self.trace:
            if math.hypot(record["logicalX"] / 1024.0, record["logicalY"] / 1024.0) <= 0.95:
                continue
            angle = math.degrees(math.atan2(record["logicalY"], record["logicalX"])) % 360
            for slot, target in enumerate(self.legacy_targets):
                if self.distance(angle, target) <= 2:
                    triples[slot] = (record["rawX"], record["rawY"], record["rawZ"])
                    selected[slot] = record["traceIndex"]
        if any(value is None for value in triples):
            return {"available": False, "reason": "legacy logical headings are incomplete"}
        return self._candidate(triples, selected, "legacy-stock-style")
    ...
    def result(self):
        ...
        return {
            ...
            "legacyCandidate": self._legacy_candidate(), "rawSpaceCandidate": proposed,
            # Compatibility fields always refer to the proposed candidate.
            "triples": proposed["triples"], "selectedTraceIndices": proposed["selectedTraceIndices"],
            "derivedWords": proposed["derivedWords"], "tableWords": proposed["tableWords"],
            "tableHex": proposed["tableHex"],
        }
...
class PreviewManager:
    ...
    def capability(self):
        available = device_path() is not None
        return {"available": available, "abiVersion": ABI_VERSION,
                "writeEnabled": available and device_compatible() == RP6_COMPATIBLE,
                "phases": sorted(PHASES), "writeDelaySeconds": WRITE_DELAY_SECONDS}
    ...
    def commit(self, token, confirmation):
        token = str(token or "")
        if confirmation != "APPLY CALIBRATION":
            raise ValueError("explicit calibration confirmation is required")
        if device_compatible() != RP6_COMPATIBLE:
            raise RuntimeError("persistent MCU calibration is not supported on this device")
        with self.lock:
            if self.session and self.session.snapshot()["active"]:
