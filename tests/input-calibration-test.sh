...
assert len(range_tracker.result()["tableWords"]) == 29
...
# An elliptical raw response and the observed +908 logical positive-Y shortfall
# must not steer raw-space selection through the old calibration. The legacy
# view is still derived from exactly the same immutable trace for comparison.
...
assert distorted_result["legacyCandidate"]["available"] is False
assert distorted_result["slotRawAngles"] == [90, 180, 270, 0, 45, 135, 225, 315]

# 72/72 coverage with four turns is insufficient when the stick never makes
# firm outer-gate contact.
...
# A3 has the same canonical raw-vector slot order as the stock RP6 block and
# A6.  Because the left logical axes are rotated 180 degrees, logical +Y must
# populate A3 triple 2 (words 6..8), not triple 0.  A regression here swaps the
# two Y ranges and reproduces a one-sided left-Y calibration failure.
...
selected = left_result["selectedTraceIndices"]
...
assert all(left_result["triples"][slot] == [left_range.trace[index][key]
...
class FakePreviewManager:
    def capability(self):
        return {"available": True, "abiVersion": 1, "writeEnabled": False}
    ...
    def commit(self, token, confirmation):
        return {"committed": token == "test" and confirmation == "APPLY CALIBRATION"}


control.RSINPUT_CALIBRATION = FakePreviewManager()
assert control.action_rsinput_calibration_capability({})["writeEnabled"] is False
...
assert control.action_rsinput_calibration_commit(
    {"token": "test", "confirmation": "APPLY CALIBRATION"}
)["committed"] is True

# The real manager rejects incomplete sessions without writing. A complete RP6
# session emits the stock-compatible eight-command order with a dwell after each
# journaled frame.
...
original_compatible = rsinput_calibration.device_compatible
...
rsinput_calibration.device_compatible = lambda: rsinput_calibration.RP6_COMPATIBLE
delays = []
rsinput_calibration.time.sleep = delays.append
rsinput_calibration.JOURNAL_ROOT = work
try:
    try:
        manager.commit("test", "APPLY CALIBRATION")
    except RuntimeError as error:
    ...
    committed = manager.commit("test", "APPLY CALIBRATION")
    ...
    rsinput_calibration.device_compatible = original_compatible
...
calibration_ui = (root / "decky/armada-control/src/components/Calibration.tsx").read_text()
assert "ProgressBar" not in calibration_ui
assert 'role="progressbar"' in calibration_ui
PYEOF
